...
namespace Microsoft.MixedReality.Toolkit.Experimental.UI
{
    ...
    public class NonNativeKeyboard : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            if (SliderEnabled)
            {
                var nearPoint = Vector3.ProjectOnPlane(Camera.main.transform.forward, transform.forward);
                var relPos = transform.InverseTransformPoint(nearPoint);
        ...
        public void RepositionKeyboard(Transform objectTransform, BoxCollider aCollider = null, float verticalOffset = 0.0f)
        {
            ...
            if (aCollider != null)
            {
                var yTranslation = -(aCollider.bounds.size.y * 0.5f + verticalOffset);
                transform.Translate(0.0f, yTranslation, -0.6f, objectTransform);
            }
            else
            {
                var yTranslation = -(m_ObjectBounds.y * 0.5f + verticalOffset);
                transform.Translate(0.0f, yTranslation, -0.6f, objectTransform);
            }
        ...
        private void ScaleToSize()
        {
            ...
            var distancePercent = (distance - m_MinDistance) / (m_MaxDistance - m_MinDistance);
            var scale = m_MinScale + (m_MaxScale - m_MinScale) * distancePercent;
        ...
        {
            if (CloseOnInactivity) _closingTime = Time.time + CloseOnInactivityTime;
        }
        ...
        {
            if (Time.time > _closingTime && CloseOnInactivity) Close();
        }
        ...
        public void Backspace()
        {
            ...
            if (InputField.selectionFocusPosition != InputField.caretPosition || InputField.selectionAnchorPosition != InputField.caretPosition)
            {
                ...
                {
                    InputField.text = InputField.text.Substring(0, InputField.selectionFocusPosition) +
                                      InputField.text.Substring(InputField.selectionAnchorPosition);
                    InputField.caretPosition = InputField.selectionFocusPosition;
                }
                else // left to right
                {
                    InputField.text = InputField.text.Substring(0, InputField.selectionAnchorPosition) +
        ...
        public void Shift(bool newShiftState)
        {
            ...
            if (m_IsCapslocked && !newShiftState) m_IsCapslocked = false;
        }
        ...
        {
            ResetKeyboardState();
            if (InputField.caretPosition != 0) InputField.MoveTextStart(false);
            InputField.text = "";
            m_CaretPosition = InputField.caretPosition;
        }
